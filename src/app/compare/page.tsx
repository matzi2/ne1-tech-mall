    ...
    <div className="bg-slate-100">
      <div className="border-b border-slate-200 bg-navy text-white">
        <div className="mx-auto max-w-6xl px-4 py-8">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-white/75">
            품번·정격으로 찾고 사양을 맞춘 뒤, 부품마다 Digi-Key·Mouser 기준으로 가격·재고를 봅니다. 비교는{" "}
            {COMPARE_MAX}개까지입니다.
          </p>
          <form
      ...
      <div className="mx-auto grid max-w-6xl gap-6 px-4 py-6 lg:grid-cols-[240px_1fr]">
        ...
        <div>
          ...
          <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-5">
            <h2 className="text-lg font-bold text-navy">Digi-Key · Mouser 가격·재고</h2>
            <p className="mt-1 text-sm text-slate-500">
              선택한 부품마다 이 몰 가격과 Digi-Key·Mouser 검색 시세를 나란히 봅니다. 검색할 때마다 다시 조회합니다.
            </p>
            {selected.length === 0 ? (
              <p className="mt-6 rounded-xl bg-slate-50 px-4 py-6 text-center text-sm text-slate-500">
                위 표에서 비교 체크를 하면 그 부품의 Digi-Key · Mouser 시세가 여기에 열립니다. 한 개만 선택해도 됩니다.
              </p>
            ) : (
              <div className="mt-4 space-y-5">
                {selected.map((item) => (
                  <SiteOfferTable key={item.slug} product={item} searchToken={filters.query} />
                ))}
              </div>
            )}
          </section>
